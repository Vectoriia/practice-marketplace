...
import { PostOrderData } from '../api/ordersApi';
...
const CartModal: FC<Props> = ({
  ...
  const handleSubmitOrder = async () => {
    ...
    if (isUserAuthorised) {
      try {
        await PostOrderData(user.token, paramArray);
        handleCheckoutOpen();
      } catch (error) {
        window.alert(error);
        navigate('/signin');
      ...
      >
        <Box sx={mainBoxStyle}>
          <div className="flex flex-col h-full">
            ...
            <Box
              className="flex flex-row w-full items-center justify-between px-[20px] mt-auto"
              sx={bottomBoxStyle}
            >
              <div>
                <Typography
