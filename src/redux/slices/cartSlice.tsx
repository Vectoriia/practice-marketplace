    ...
    editItemCount: (state, action: PayloadAction<EditItemProps>) => {
      ...
      if (index >= 0) {
        ...
        if (tmpItem.amount + action.payload.operator !== 0) {
          state.items[index] = {
            ...state.items[index],
            amount: tmpItem.amount + action.payload.operator,
          };
        }
      }
    },
...
export const { addItemToCart, deleteItemFromCart, editItemCount, clearCart } =
  cartSlice.actions;

// Other code such as selectors can use the imported `RootState` type
export const selectCart = (state: RootState) => state.cart.items;
export const getCartTotalPrice = (state: RootState) =>
  state.cart.items.reduce((acc, next) => (acc += next.amount * next.price), 0);
